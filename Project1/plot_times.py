...
def makeplots(dicty):
    ...
    return np.array(n), np.array(val)
...
c_dict=makedict(c)
e_dict=makedict(e)
n,eval=makeplots(e_dict)
plt.plot(np.log10(n),np.log10(eval),label="LU decomposition of general matrix")
n,bval=makeplots(b_dict)
plt.plot(np.log10(n),np.log10(bval),label="general triangular algorithm")
n,cval=makeplots(c_dict)
plt.plot(np.log10(n),np.log10(cval),label="specialized algorithm")
