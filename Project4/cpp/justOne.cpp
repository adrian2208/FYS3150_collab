...
int main(int argc, char** argv){
  ...
  double all_results_total[1*tot_temp][12]; // Array storing all results for all threads when calling MPI_Reduce
  for(int i=0; i<1*tot_temp;i++){
    for(int j=0;j<12;j++){
      all_results_total[i][j]=0;
    }
  }

  double all_results[1*tot_temp][12]; // Array storing all results (numprocs-1)/numprocs will remain empty
  for(int i=0; i<1*tot_temp;i++){
    for(int j=0;j<12;j++){
      all_results[i][j]=0;
    }
