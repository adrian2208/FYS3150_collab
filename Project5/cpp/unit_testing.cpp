...
TEST_CASE("Test that alpha=1.0 is the ideal solution when there's no repulsion"){
  double alpha=1.0, beta=0,omega=1,time=0,dr=1;int samplings=1e7,skip=1e6;
  ...
  double energy_alpha=0,energysquared_alpha=0;
  double sigma_alpha;
    vrc.sample(&energy_alpha,&energysquared_alpha,&time,pos);
  ...
  for (double alpha=0.1;alpha<2;alpha+=0.2){
    ...
    vrc=VRMonteCarlo(&per, dr,samplings,skip,0);
